...
public class EntityAIZigZagTarget extends EntityAIBase
{
    ...
    public boolean shouldExecute()
    {
        ...
        else
        {
        	...
        	if (pathentity != null)
        	{
        		if (pathentity.getCurrentPathLength() >= this.maxTargetDistance)
		        {
		        	PathPoint pathpoint = pathentity.getPathPointFromIndex(MathHelper.floor_double(this.maxTargetDistance / 2.0D));
		        	this.entityPosX = pathpoint.xCoord;
		        	this.entityPosY = pathpoint.yCoord;
		        	this.entityPosZ = pathpoint.zCoord;
		        	
		        	Vec3 vec3 = RandomPositionGeneratorAlt.findRandomTargetBlockTowards(this.theEntity, (int)maxTargetDistance, 3, Vec3.createVectorHelper(this.entityPosX, this.entityPosY, this.entityPosZ));
...
//		            	if (this.theEntity.getDistanceSq(vec3.xCoord, vec3.yCoord, vec3.zCoord) > maxTargetDistance * maxTargetDistance && this.targetEntity.getDistanceSq(vec3.xCoord, vec3.yCoord, vec3.zCoord) < this.targetEntity.getDistanceSq(entityPosX, entityPosY, entityPosZ))
		            	{
